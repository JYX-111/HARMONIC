# -*- coding: utf-8 -*-
"""
Cell-cell communication (CCC) visualization for baseline method (without texture).

This script generates visualizations of CCC networks with:
- Simple background without textures
...
def process_single_out_dir(out_dir_name: str, output_base: str):
    """Process a single without_texture output directory."""
    strength = out_dir_name.replace("without_texture_", "")
    if strength == '0':
        strength = '00'

    out_root = os.path.join(output_base, out_dir_name)
    dataset_root = f"data/permissive_p{strength}/simulation_data"
    niche_save_root = f"viz/{out_dir_name}"
    ...
    print(f"\n{'='*60}")
    print(f"[Processing] {out_dir_name}, strength={strength}")
    print(f"{'='*60}")
    ...
    for _, row in layout.iterrows():
        ...
        lw_n = float(ARROW_LINEWIDTH)
        ms_n = float(ARROW_MUTATION_SCALE)
    ...
    if len(all_cell_x) == 0:
        print(f"[warn] No niche samples loaded for {out_dir_name}. Check OUT_ROOT and niche_layout.csv nid mapping.")
        return
    ...
    lw_g = float(ARROW_LINEWIDTH)
    ms_g = float(ARROW_MUTATION_SCALE)
    ...
    global_png = os.path.join(niche_save_root, "ccc_global.png")
    global_svg = os.path.join(niche_save_root, "ccc_global.svg")
    fig.savefig(global_png, dpi=dpi_g)
    fig.savefig(global_svg)
    plt.close(fig)

    print(f"[done] {out_dir_name}: saved to {niche_save_root}")
...
def main():
    ...
    if not os.path.exists(output_base):
        print(f"[warn] Output directory not found: {output_base}")
        return

    without_texture_dirs = []
    for d in os.listdir(output_base):
        if d.startswith("without_texture_"):
            without_texture_dirs.append(d)
    without_texture_dirs = sorted(without_texture_dirs)

    if len(without_texture_dirs) == 0:
        print("[warn] No without_texture_* directories found in output/")
        return

    print(f"[info] Found without_texture dirs: {without_texture_dirs}")

    for out_dir in without_texture_dirs:
        process_single_out_dir(out_dir, output_base)

    print(f"\n[all done] Processed {len(without_texture_dirs)} directories")
